...
public class PhicommGameCore extends Thread {
	...
	public PhicommGameCore(PhicommBuyInfo phicommBuyInfo){
		this.phicommBuyInfo = phicommBuyInfo;
	}
	
	@Override
	public void run() {
		if(null != phicommBuyInfo && null != PhicommFlow.PHICOMM_GAME_FLOW && PhicommFlow.PHICOMM_GAME_FLOW.size() > 0){
			FlowUtils.info(String.format("----IP:%s----手机:%s----开始刷分---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
			...
			phicommBuyInfo.setBuyType(PhicommConstants.BUY_TYPE_GAME);
			PhicommThreadPool.PHICOMM_GAME_HTREAD_POOL.put(phicommBuyInfo.getPhone(), this);
			for (Map<String, Object> flowMap : PhicommFlow.PHICOMM_GAME_FLOW) {
				if(null == flowMap){
					continue;
				}
			...
			PhicommThreadPool.PHICOMM_GAME_HTREAD_POOL.remove(phicommBuyInfo.getPhone());
			FlowUtils.info(
					String.format("----IP:%s----手机:%s----结束刷分---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
		}
	}
