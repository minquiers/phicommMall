...
public class PhicommGameDDCow extends Thread {
	...
	public PhicommGameDDCow(PhicommBuyInfo phicommBuyInfo){
		this.phicommBuyInfo = phicommBuyInfo;
	}
	
	@Override
	public void run() {
		if(null != phicommBuyInfo && null != PhicommFlow.PHICOMM_GAMEDDCOW_FLOW && PhicommFlow.PHICOMM_GAMEDDCOW_FLOW.size() > 0){
			FlowUtils.info(String.format("----IP:%s----手机:%s----开始天天牛---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
			...
			phicommBuyInfo.setBuyType(PhicommConstants.BUY_TYPE_DDCOW);
			PhicommThreadPool.PHICOMM_GAMETTCOW_HTREAD_POOL.put(phicommBuyInfo.getPhone(), this);
			for (Map<String, Object> flowMap : PhicommFlow.PHICOMM_GAMEDDCOW_FLOW) {
				if(null == flowMap){
					continue;
				}
			...
			PhicommThreadPool.PHICOMM_GAMETTCOW_HTREAD_POOL.remove(phicommBuyInfo.getPhone());
			FlowUtils.info(
					String.format("----IP:%s----手机:%s----结束天天牛---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
		}
	}
