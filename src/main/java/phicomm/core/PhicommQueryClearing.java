...
public class PhicommQueryClearing extends Thread {
private HttpClientContext context = new HttpClientContext();
	...
	public PhicommQueryClearing(PhicommBuyInfo phicommBuyInfo){
		this.phicommBuyInfo = phicommBuyInfo;
	}
	
	@Override
	public void run() {
		if(null != phicommBuyInfo && null != PhicommFlow.PHICOMM_QUERY_CLEARING_FLOW && PhicommFlow.PHICOMM_QUERY_CLEARING_FLOW.size() > 0){
			FlowUtils.info(String.format("----IP:%s----手机:%s----开始查询结算---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
			...
			phicommBuyInfo.setBuyType(PhicommConstants.BUY_TYPE_QUERY_CLEARING);
			for (Map<String, Object> flowMap : PhicommFlow.PHICOMM_QUERY_CLEARING_FLOW) {
				if(null == flowMap){
					continue;
				}
			...
			FlowUtils.info(
					String.format("----IP:%s----手机:%s----结束查询结算---", phicommBuyInfo.getIp(), phicommBuyInfo.getPhone()));
		}
	}
