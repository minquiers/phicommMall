...
public class VisitLoginPage extends FlowBase {
	...
	public String before(HttpConfig httpConfig , PhicommBuyInfo phicommBuyInfo, Map<String,Object> flowMap){
		...
		try{
			...
			}else if(PhicommConstants.BUY_TYPE_COUPON.equals(phicommBuyInfo.getBuyType()) || PhicommConstants.BUY_TYPE_GAME.equals(phicommBuyInfo.getBuyType()) || PhicommConstants.BUY_TYPE_QUERY_CLEARING.equals(phicommBuyInfo.getBuyType())){
			...
			PhicommLogInfo.logInfo("put", phicommBuyInfo.getPhone(), "开始访问登录页面",phicommBuyInfo.getBuyType());
	...
	public String exec(HttpConfig httpConfig , PhicommBuyInfo phicommBuyInfo, Map<String,Object> flowMap , String beforeResult){
		try{
			if(PhicommConstants.FLOW_SUCCESS.equals(beforeResult)){
				...
				PhicommLogInfo.logInfo("put", phicommBuyInfo.getPhone(), "访问登录页面中",phicommBuyInfo.getBuyType());
				execRequestResult = HttpClientUtil.get(httpConfig.url(String.format("%s/passport-login.html", PhicommConstants.PHICOMM_HOST)));
			}
		}catch(Exception e){
			...
			PhicommLogInfo.logInfo("put", phicommBuyInfo.getPhone(), String.format("访问登录页面失败,错误信息:%s", e.getMessage()),phicommBuyInfo.getBuyType());
			if("break".equals(flowMap.get("errorRepeatExec"))){
		...
		try{
			if(PhicommConstants.FLOW_SUCCESS.equals(beforeResult)){
				...
				if(StringUtils.isNotBlank(execRequestResult)){
					...
					if(null != document && null != document.getElementById("input_uname")){
						beforeResult = PhicommConstants.FLOW_BREAK;
					}
				}
				if(PhicommConstants.FLOW_BREAK.equals(beforeResult)){
					PhicommLogInfo.logInfo("put", phicommBuyInfo.getPhone(), "访问登录页面成功",phicommBuyInfo.getBuyType());
				}else{
					PhicommLogInfo.logInfo("put", phicommBuyInfo.getPhone(), String.format("访问登录页面失败返回:%s", execRequestResult) ,phicommBuyInfo.getBuyType() );
				}
			}
		}catch(Exception e){
