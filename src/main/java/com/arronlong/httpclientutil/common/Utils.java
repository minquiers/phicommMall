...
public class Utils {
	...
	public static final String ENTITY_XML = "$ENTITY_XML$";
	...
	private static final List<String> SPECIAL_ENTITIY = Arrays.asList(ENTITY_STRING, ENTITY_XML, ENTITY_BYTES, ENTITY_FILE, ENTITY_INPUTSTREAM, ENTITY_SERIALIZABLE, ENTITY_MULTIPART,ENTITY_JSON);
			...
			for (Entry<String, Object> entry : map.entrySet()) {
				if(SPECIAL_ENTITIY.contains(entry.getKey())){//判断是否在之中
					...
					if(ENTITY_STRING.equals(entry.getKey())){//string
						...
						break;
					}else if(ENTITY_XML.equals(entry.getKey())){//xml
						entity = new StringEntity(String.valueOf(entry.getValue()) , ContentType.create("text/xml", encoding));
						break;
					}else if(ENTITY_JSON.equals(entry.getKey())){//json
						entity = new StringEntity(String.valueOf(entry.getValue()) , ContentType.create("application/json", encoding));
						break;
						...
						if(File.class.isAssignableFrom(entry.getValue().getClass())){
							...
							entity = new FileEntity(new File((String) entry.getValue()), ContentType.create("text/plain", encoding));
						}
						break;
