...
import com.arronlong.httpclientutil.logs.HttpClientLog;
...
public class HttpClientUtil{
	...
	static{
		try {
			client4HTTP = HCB.custom().timeout(20000).build();
			client4HTTPS = HCB.custom().timeout(20000).ssl().build();
		} catch (HttpProcessException e) {
			HttpClientLog.errorException("创建https协议的HttpClient对象出错：{}", e);
		}
	}
	...
	private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
		...
		try {
			...
			if(HttpEntityEnclosingRequestBase.class.isAssignableFrom(request.getClass())){
				...
				HttpClientLog.info("请求地址："+config.url());
				if(nvps.size()>0){
					HttpClientLog.info("请求参数："+nvps.toString());
				}
				if(config.json()!=null){
					HttpClientLog.info("请求参数："+config.json());
				}
				HttpClientLog.info("请求内容："+EntityUtils.toString(entity));
			}else{
				int idx = config.url().indexOf("?");
				HttpClientLog.info("请求地址："+config.url().substring(0, (idx>0 ? idx : config.url().length())));
				if(idx>0){
					HttpClientLog.info("请求参数："+config.url().substring(idx+1));
				}
			}
			//执行请求操作，并拿到结果（同步阻塞）
			...
			if(null != config.context()){
				if(config.context() instanceof HttpClientContext){
					HttpClientContext httpClientContext = (HttpClientContext) config.context();
					if(null != httpClientContext.getCookieStore() && null != resp.getAllHeaders() && resp.getAllHeaders().length > 0){
						for(Header header : resp.getAllHeaders()){
							if(header.getName().equals("Set-Cookie")){
								String cookieValue = header.getValue();
								if(StringUtils.isNotBlank(cookieValue)){
									String cookieAllInfo[] = cookieValue.split(";");
									if(null != cookieAllInfo && cookieAllInfo.length > 0){
										String cookieNameInfo[] = cookieAllInfo[0].split("=");
										String cookiePathInfo[] = cookieAllInfo[1].split("=");
										//String cookieExpiresInfo[] = cookieAllInfo[2].split("=");
										if(null != cookieNameInfo && cookieNameInfo.length > 1){
											boolean isExistsCookie = false;
											for (Cookie storeCookie : httpClientContext.getCookieStore().getCookies()) {
												if(storeCookie.getName().equals(cookieNameInfo[0])){
													isExistsCookie = true;
													break;
												}
											}
											if(!isExistsCookie){
												BasicClientCookie cookie = new BasicClientCookie(cookieNameInfo[0], cookieNameInfo[1]);
												if(null != cookiePathInfo && cookiePathInfo.length > 1){
													if(cookiePathInfo[1].equals("/")){
														String pattern = "[^//]*?\\.(com|cn|net|org|biz|info|cc|tv)" ;
												        Pattern p = Pattern.compile(pattern,Pattern.CASE_INSENSITIVE);
												        Matcher matcher = p.matcher(config.url());
												        matcher.find();
														cookie.setDomain(matcher.group());
													}else{
														cookie.setDomain(cookiePathInfo[1]);
													}
													
												}
												/*if(null != cookieExpiresInfo && cookieExpiresInfo.length > 1){
													SimpleDateFormat sdf = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.US);
													try{
														cookie.setExpiryDate(sdf.parse(cookieExpiresInfo[1]));
													}catch(Exception e){
	...
	private static void close(HttpResponse resp) {
		try {
			...
			HttpClientLog.errorException(e.getMessage(),e);
		}
	}
}